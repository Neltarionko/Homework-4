...
# Функция main
def main(radiobeacons, pseudorange,tau):
    x = []
    y = []
    for i in range(len(radiobeacons)):
    ...
    for i in range(len(pseudorange)):
        noise.append(pseudorange[i]-tau[0]-math.sqrt((x0 - radiobeacons[i][0])**2 + (y0 - radiobeacons[i][1])**2))
        print('Процент шума от значения псевдодальности:', noise[i]/pseudorange[i]*100)
    ...
    for i in range(len(radiobeacons)):
        ...
        globals()['circle_%s' % i] = plt.Circle(radiobeacons[i], pseudorange[i]-tau[0]-noise[i], color='b', fill=False, linestyle='--')
