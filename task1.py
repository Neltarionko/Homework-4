...
# Входные данные
radiobeacons = [(-50,100),(300,500),(-200,-700),(200,-600),(50,1000)]
pseudorange = [149.4139, 330.6324, 917.4691, 775.1122, 771.5541]
...
for i in range (len(list_of_inter_new)):
    ...
    for k in range (len(list_of_inter_new) - 1, i, -1):
        if math.sqrt((list_of_inter_new[k][0] - a[0])**2 + (list_of_inter_new[k][1] - a[1])**2) < 9:
...
# Нахождение шума измерений
noise = []
for i in range(len(pseudorange)):
    noise.append(math.sqrt((x0 - radiobeacons[i][0])**2 + (y0 - radiobeacons[i][1])**2) - pseudorange[i])
print(noise) 

# Нахождение погрешности часов из любого из уравнений
tau = pseudorange[0] + noise[0] - math.sqrt((x0 - radiobeacons[0][0])**2 + (y0 - radiobeacons[0][1])**2)
print(tau) 
...
plt.scatter(x0, y0)
for i in range(len(radiobeacons)):
    globals()['circle%s' % i] = plt.Circle(radiobeacons[i], pseudorange[i], color='b', fill=False, linestyle='--')
    plt.gca().add_artist(globals()['circle%s' % i])
    globals()['circle_%s' % i] = plt.Circle(radiobeacons[i], pseudorange[i]+noise[i], color='b', fill=False)
